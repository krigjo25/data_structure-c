...
bool check(char* word)
{
    node *trav= root; //  Point the node to the root

    //  Iterate through the length of the word
    for (int i = 0; i < strlen(word); i++)
    {
        int i = tolower(word[i])- 'a';//  find index for the letter case-insensitive

        if (!trav)   //  Check if the node doesn't exist
        {
            return false;
        }

        trav = trav->children[i]; // reset the cur to this node
